# Installing OKD 4.11 on KVM using Terraform
...
The utility server is configured using an ignition, which is generated by terraform using haproxy.conf and haproxy.service input files in
the terraform working directory.

As load-balancing is done differently depending on your deployment scenario, the haproxy.conf is provided in two different versions.
One for the three master, three worker node scenario (haproxy-3workers.conf), and one for the three master nodes only scenario
(haproxy-onlymasters.conf).

Copy the file you want to haproxy.conf, and customize it to your needs, updating IP addresses, domainnames, etc.
